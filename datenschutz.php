...
$db = getDB();
$stmt = $db->prepare("SELECT * FROM legal_pages WHERE slug='datenschutz'");
$stmt->execute();
$page = $stmt->fetch();
?>
<div class="container" style="max-width:760px;padding:60px 24px;">
    <nav style="font-size:0.85rem;color:var(--gray-400);margin-bottom:32px;">
        <a href="/" style="color:var(--gray-400);">Startseite</a> / <?= e($page['title'] ?? 'Datenschutz') ?>
    </nav>
    <?php if ($page): ?>
    ...
    <?php else: ?>
        ...
        <p style="color:var(--gray-500);">Die Datenschutzerklärung wird gerade eingerichtet.</p>
    <?php endif; ?>
</div>
<style>
