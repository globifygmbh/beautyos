...
$db = getDB();
$stmt = $db->prepare("SELECT * FROM legal_pages WHERE slug='agb'");
$stmt->execute();
$page = $stmt->fetch();
?>
<div class="container" style="max-width:760px;padding:60px 24px;">
    <nav style="font-size:0.85rem;color:var(--gray-400);margin-bottom:32px;">
        <a href="/" style="color:var(--gray-400);">Startseite</a> / <?= e($page['title'] ?? 'AGB') ?>
    </nav>
    <?php if ($page): ?>
    ...
    <?php else: ?>
        <h1>AGB</h1>
        <p style="color:var(--gray-500);">Die AGB werden gerade eingerichtet.</p>
    <?php endif; ?>
</div>
<style>
